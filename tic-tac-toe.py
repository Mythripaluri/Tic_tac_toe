    ...
    for row in range(BOARD_ROWS):
        for col in range(BOARD_COLS):
            if board[row][col]==1:
                ...
                pygame.draw.line(screen, color, (col*SQUARE_SIZE + SQUARE_SIZE*3//4, row*SQUARE_SIZE + SQUARE_SIZE//4), (col*SQUARE_SIZE + SQUARE_SIZE*1//4, row*SQUARE_SIZE + SQUARE_SIZE*3//4), CROSS_WIDTH)
...
def check_win(player,check_board=board):
    ...
    for col in range(BOARD_COLS):
        if check_board[0][col]==check_board[1][col]==check_board[2][col]==player:
            return True
    # Check diagonals
    if check_board[0][0]==check_board[1][1]==check_board[2][2]==player or check_board[0][2]==check_board[1][1]==check_board[2][0]==player:
        return True
    return False


def minimax(minimax_board,depth,is_maximizing):
    # Check if the current board is a terminal state (win, lose, tie)
